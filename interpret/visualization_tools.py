...
def CLUE_viewer(Nim, x_init_batch, y_init_batch, x_vec, BNN, VAE_regen=False, img_progress=False, \
                loss_curves=False, img_delta=True, tgets=np.array(range(10)), \
                cost_vec=None, aleatoric_vec=None, epistemic_vec=None, dist_vec=None):
    ...
    if img_delta:
        ...
        to_BNN = MNIST_mean_std_norm(torch.tensor(x_init_batch[N_explain, 0, :, :]).view(1, -1))
        probs = BNN.sample_predict(to_BNN, Nsamples=0, grad=False)
        total_entropy, o_aleatoric_entropy, o_epistemic_entropy = decompose_entropy_cat(probs)
        ...
        to_BNN = MNIST_mean_std_norm(torch.tensor(x_vec[-1, N_explain, 0, :, :]).view(1, -1))
        probs = BNN.sample_predict(to_BNN, Nsamples=0, grad=False)
        total_entropy, aleatoric_entropy, epistemic_entropy = decompose_entropy_cat(probs)
...
def FIDO_viewer(Nim, x_init_batch, explainer, BNN, VAEAC, tgets=np.array(range(10))):
    ...
    explanation, mask = explainer.mask_inpaint(x_init_batch, VAEAC, flatten_ims=True, test_dims=10, cat=True)
    ...
    mask_pos = np.repeat(np.expand_dims(mask[Nim, :].view(28, 28).data.cpu().numpy(), axis=2), 4, axis=2)
    mask_pos[:, :, 1:3] = 0
...
def CLUE_viewer_ext(Nim, x_init, true_labels, x_explain, o_aleatoric_entropy, o_epistemic_entropy, o_preds,
                    aleatoric_entropy, epistemic_entropy, preds, tgets=np.array(range(10))):
    ...
    ax[0].set_title('a %2.3f e %2.3f' % (o_aleatoric_entropy[N_explain], o_epistemic_entropy[N_explain]))
    ax[0].set_xlabel(tgets[o_preds[N_explain].astype(int)])

    ax[1].imshow(1 - x_explain[N_explain, 0, :, :], cmap='gray')
    ax[1].set_title('a %2.3f e %2.3f' % (aleatoric_entropy[N_explain], epistemic_entropy[N_explain]))
