...
class CounterfactualOptimizer:
    """
    A counterfactual optimizer that optimizes a single latent point
    directly in the intermediate feature space (latent space) produced, for example, by the
    encoder of your classifier_decoders.Classifier. The objective is a combination of reducing the
    ...
    def __init__(self, classifier, z0, uncertainty_weight=1.0, distance_weight=1.0, lr=0.1, device='cpu', 
                 bayesian=False, verbose=True, target_class=None, autoencoder=None):
        """
        Args:
            ...
            autoencoder: Optional autoencoder model. If provided, enables auxiliary optimization
        ...
        self.autoencoder = autoencoder.to(self.device) if autoencoder is not None else None
    ...
    def optimize(self, steps=25):
        ...
        if self.target_class is not None:
            print(f"Final target class probability: {target_prob.item():.4f}, Distance: {distance.item():.4f}")
        else:
            print(f"Final total entropy: {total_entropy.item():.4f}, Epistemic Entropy: {epistemic_entropy.item():.4f}, Aleatoric Entropy: {aleatoric_entropy.item():.4f}, Distance: {distance.item():.4f}")

        return self.z.detach()
    
    def auxiliary_optimize(self, steps=25, num_samples=None):
        """
        Optimizes the latent code in the autoencoder's latent space by minimizing the objective:
            If target_class is None:
                loss = uncertainty_weight * H(y|x) + distance_weight * ||z - z0||_2
            If target_class is specified:
                loss = uncertainty_weight * (-log(p[target_class])) + distance_weight * ||z - z0||_2
        
        Where x = autoencoder.decode(z) and the classifier operates on x directly.
        ...
        if self.autoencoder is None:
            raise ValueError("Autoencoder must be provided for auxiliary optimization.")
        ...
        for step in range(steps):
            ...
            # Decode the latent code to get the input for the classifier
            x = self.autoencoder.decode(self.z)
            
            if self.target_class is not None:
                # Optimize for target class
                if self.bayesian:
                    # For Bayesian case, maximize probability of target class
                    probs = self.classifier.sample_predict(x, num_samples)  # [num_samples, 1, num_classes]
                    posterior_preds = probs.mean(dim=0, keepdim=False)  # [1, num_classes]
                    target_prob = posterior_preds[0, self.target_class]
                    # Negative log probability (to be minimized)
                    target_term = -torch.log(target_prob + 1e-10)
                    loss = self.uncertainty_weight * target_term + self.distance_weight * distance
                ...
                # Original entropy-based optimization
                if self.bayesian:
                    # Get samples from Bayesian model
                    probs = self.classifier.sample_predict(x, num_samples)  # [num_samples, 1, num_classes]
                    ...
                    # For non-Bayesian case
                    logits = self.classifier(x)
                    probs = torch.nn.functional.softmax(logits, dim=1)
                    total_entropy = -(probs * torch.log(probs + 1e-10)).sum(dim=1)
                    aleatoric_entropy = total_entropy  # For non-Bayesian case, no uncertainty decomposition
                    epistemic_entropy = torch.tensor(0.0).to(self.device)
